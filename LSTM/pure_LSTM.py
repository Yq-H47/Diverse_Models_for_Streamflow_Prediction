...
train_x1, train_y = generate_train_test(sum_result, sum_result1, wrap_length=wrap_length)

print(f'The shape of train_x, train_x1, train_y after wrapping by {wrap_length} days are:')
print(f'{train_x1.shape}, {train_y.shape}')

#Data-driven models are coupled to different process-based models
def create_model(input_xd_shape1, seed):
    ...
    xd_hydro = Concatenate(axis=-1, name='Concat')(xd_input_forprnn1)

    e_hn, e_cn = LSTM_postprocess(input_xd = 20, hidden_size=256, seed=seed, name='LSTM')(xd_hydro)
    fc2_out = Dense(units=1)(e_hn)
    fc2_out = K.permute_dimensions(fc2_out, pattern=(1,0,2))  # for test model

    model = Model(inputs=xd_input_forprnn1, outputs=fc2_out)
    return model


def train_model(model, train_xd1, train_y, ep_number, lrate, save_path):
    ...
    history = model.fit(x=train_xd1, y=train_y, epochs=ep_number, batch_size=580,
...
model = create_model(input_xd_shape1=(train_x1.shape[1], train_x1.shape[2]), seed= 200)
model.summary()

train_history = train_model(model=model,train_xd1=train_x1, train_y=train_y, ep_number=150, lrate=0.01, save_path=save_path)
